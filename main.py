...
DQgQIECAwN8S+DdA/oF/DQgQIECAwN8S+DdA/oF/DQgQIECAwN8S+DdA/oF/DQgQIECAwN8S+DdA
/oF/DQgQIECAwN8S+DdA/oF/DQgQIECAwN8S+DdA/oF/DQgQIECAwN8S+DdA/oF/DQgQIECAwN8S
+DdA/oF/DQgQIECAwN8S+DdA/oF/DQgQIECAwN8S+P0D28u7vB1g5cAAAAAASUVORK5CYII=
"""
...
def save_performance_metrics():
    ...
    filename = f"Performance/LBPH_Performance_{timestamp}.csv"
    ...
    summary_filename = f"Performance/LBPH_Summary_{timestamp}.csv"
...
def getImagesAndLabels(path):
    ...
    for imagePath in imagePaths:
        try:
            ...
            print(f"Skipping malformed file: {imagePath}")
    return faces, Ids
...
def get_attendance_status(user_id, login_status, current_time):
    ...
    if last_user_status['status'] == 'Login':
        if time_since_last_event.total_seconds() > 30:
            return 'Logout'
...
class AttendanceWindow(tk.Toplevel):
    def __init__(self, on_closing_callback, *args, **kwargs):
        ...
        self.title("Face Attendance - LBPH")
        ...
        tk.Label(header_frame, text="Real-Time Attendance (LBPH)", font=("Helvetica", 22, "bold"), bg="#34495e", fg="#ecf0f1").pack(pady=10)
...
def TrackImages():
    ...
    def video_loop():
        ...
        for (x, y, w, h) in faces:
            ...
            cv2.rectangle(im, (x, y), (x + w, y + h), (255, 165, 0), 2)
            ...
            if conf < 75:
                ...
                performance_metrics['confidence_scores'].append(100 - conf)
                ...
                cv2.putText(im, name, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 165, 0), 2)
                ...
                attendance_queue.put({'action': 'update_info', 'name': name, 'user_id': user_id, 'status': display_status, 'confidence': 100-conf, 'processing_time': processing_time})
...
def update_performance_display():
    ...
    if total > 0:
        ...
        if performance_metrics['confidence_scores']:
            ...
            perf_conf_label.config(text=f"Avg Confidence: {avg_conf:.2f}")
            ...
            if os.path.isdir(img_folder):
                for filename in os.listdir(img_folder):
                    try:
                        ...
                        continue
            if removed_images > 0:
                mess.showinfo("Images Deleted", f"Deleted {removed_images} images for ID {del_id}.")
...
window.title("Smart Face Recognition Attendance System - LBPH")
...
title_label = tk.Label(top_bar, text="Smart Attendance System (LBPH)", font=FONT_TITLE, bg=BG_COLOR)
title_label.pack()
...
label_del = ttk.Label(tab_delete, text="Enter ID to delete user details and images", style="Header.TLabel")
...
# NEW: Performance Dashboard Tab
tab_performance = ttk.Frame(notebook, padding=25)
notebook.add(tab_performance, text='Performance')
ttk.Label(tab_performance, text="Performance Metrics (LBPH Method)", style="Header.TLabel").pack(anchor="w", pady=(0, 20))
